...
import { useClients, useDeleteClient } from '@/hooks/useData';
...
export default function Clients() {
  ...
  const handleDelete = (client) => {
    if (window.confirm(`هل أنت متأكد من حذف "${client.name}"؟\nسيتم حذف جميع المشاريع المرتبطة أيضاً.`)) {
  ...
  return (
    <PageTransition className="space-y-6">
      <ClientForm isOpen={showForm} onClose={() => { setShowForm(false); setEditingClient(null); }} initialData={editingClient} />
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {isOwner && (
          <Button onClick={() => { setEditingClient(null); setShowForm(true); }} className="bg-accent hover:bg-accent-hover text-white font-sans flex items-center gap-2 pr-4">
